#!/bin/sh 
# setup: add our library to the $PATH (*); and do chmod 777 unstring_delimited_by_colons.sh so as to overcome the Permission Denied error;
...
# invocation: [bash] unstring_delimited_by_colons.sh
# Notes: 
...
#
cat input_file_delimited_by_colons  # display the original file
awk '/^:/{getline x;getline;next}{print > x}' input_file_delimited_by_colons
